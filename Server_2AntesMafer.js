...
app.post('/DeleteUser', function (req, res) {
    ...
    MyMongo.Remove('Usuarios', { Email: req.body.Email }, function (result) {
        var Data = {};
        Data.Result = 'Ok';
        res.end(JSON.stringify(Data));
    }
    ...
    if (req.body.EditUser == '') {
        MyMongo.Find('Usuarios', { Email: req.body.Email }, function (result) {
            if (result.length > 0) {
                ...
                MyMongo.Insert('Usuarios', { Email: req.body.Email, Password: req.body.Password, Level: req.body.Level, Perfil: req.body.Perfil, NombrePerfil: req.body.NombrePerfil, Name: req.body.UserName, User: req.body.User, Nit: req.body.Nit, Phone: req.body.Phone }, function (result) {

                    var msg = MyConst.CorreoHTML;

                    msg = msg.replace('@@Encabezado', 'Acceso a plataforma de proveedores de Proenfar.');

                    var msgBody = 'Puede ingresar a la plataforma de recibos de Proenfar con éste correo y clave: ' + req.body.Password;
        ...
        MyMongo.UpdateCriteria('Usuarios', { Email: req.body.EditUser }, { Password: req.body.Password, Level: req.body.Level, Perfil: req.body.Perfil, NombrePerfil: req.body.NombrePerfil, Name: req.body.UserName, User: req.body.User, Nit: req.body.Nit, Phone: req.body.Phone }, function (resp) {
            Data.Result = 'Ok';
            res.end(JSON.stringify(Data))
            return 0;
        }
  ...
  var Data = {};
    ...
    MyMongo.Find('Usuarios', { Email: req.body.Email }, function (result) {
      if (result.length == 0){
        ...
        MyMongo.UpdateCriteria('Usuarios', { 'Email': req.body.Email }, { 'Password': text }, function (result) {
            if (result == 'Ok') {
                MyMail.SendEmail("Esta es su nueva clave: " + text + "</b><p>", req.body.Email, "Recuperacion de acceso");
                Data.Result = 'Ok';
                res.end(JSON.stringify(Data))
            };
        }
...
app.post('/GetUsuarioProveedor', function (req, res) {
  MyMongo.Find('Usuarios', { NombrePerfil: req.body.NombrePerfil }, function (result) {
    var Data = {};
    Data.Result = 'Ok';
    Data.data = result;
    res.end(JSON.stringify(Data));
...
app.post('/EnviarEmailProveedores', function (req, res) {
    ...
    req.body.Proveedores.forEach(function(element) {
      ...
      var msgCorreo = '<table style="max-width: 600px; padding: 10px; margin:0 auto; border-collapse: collapse;">'+
      '  <tbody>'+
      '    <tr>'+
      ...
      '                    <td align="center">'+
      '                      <br>'+
      '                      <p style="font-family: sans-serif;font-size:11px;line-height:20px;color:#333333;padding-left:5px;margin:0 0 10px 0;text-align:left;font-family:Helvetica,Arial,sans-serif">'+
      '                        <b>Nota:</b>El proceso de Licitación estara disponible desde.'+
      '                      </p>'+
      '                    </td>'+
      '                  </tr>'+
      '                  <tr>'+
      '                    <td>'+
      '                      <p style="font-family: sans-serif;font-size:12px;padding-left:5px;margin:0 0 20px 0;">06/11/2017 hasta el 28/11/2017, agradecemos su atención.<br>'+
      ...
      msgCorreo = msgCorreo.replace('@@Usuario', element.Email);
      ...
      MyMail.SendEmail(msgCorreo, element.Email, subject);
...
app.post('/Login', function (req, res) {
    ...
    MyMongo.Find('Usuarios', { $and: [{ "Email": email }, { "Password": password }] }, function (result) {
      ...
      if (result.length == 0){
        ...
        MyMongo.Find('Aduanas', {}, function (result) {
          ...
          Data.ModalidadesProveedor = {
            ...
            Otm: {
              Otms:[
                {Origen: 'Cartagena', Destino: 'Bogota Zona Franca', cveintecuatroconcinco: 0, cveinteoocho: 0, cveintendiez: 0, cveintendiezsiete: 0, cveintendieznueve: 0, cveinteveinte: 0, cveinteveinteyuno: 0, cveinteveinteycinco: 0, ccuarentaquince: 0, ccuarentaquince: 0, ccuarentadiezyseis: 0, ccuarentadiezysiete: 0, ccuarentaveinte: 0, ccuarentaveinteyuno: 0, ccuarentaveinteydos: 0, ccuarentatreinta: 0, devolucionveinteestandar: 0, devolucioncuarentaestandar: 0, devolucionveinteexpreso: 0, devolucioncuarentaexpreso: 0},
                {Origen: 'Buenaaventura', Destino: 'Bogota Zona Franca', cveintecuatroconcinco: 0, cveinteoocho: 0, cveintendiez: 0, cveintendiezsiete: 0, cveintendieznueve: 0, cveinteveinte: 0, cveinteveinteyuno: 0, cveinteveinteycinco: 0, ccuarentaquince: 0, ccuarentaquince: 0, ccuarentadiezyseis: 0, ccuarentadiezysiete: 0, ccuarentaveinte: 0, ccuarentaveinteyuno: 0, ccuarentaveinteydos: 0, ccuarentatreinta: 0, devolucionveinteestandar: 0, devolucioncuarentaestandar: 0, devolucionveinteexpreso: 0, devolucioncuarentaexpreso: 0}
              ]
            }
            };
        ...
        Data.ModalidadesProveedor = result[0];
        res.end(JSON.stringify(Data));
      }
    } );
...
console.log("Running at Port 8091 Ok");
